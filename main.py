...
def main(version, input1):
    ...
    for atoms in init_assignments: assments.append(atoms)
    for unit in init_units: units.append(unit)
...
if __name__ == "__main__":
    ...
    for file in glob.glob(example + "/*.txt"): # ("*.txt") for single file, otherwise directory
        ...
        initial_assignments, assignments, message, backtrack_counter, units = main(version, file)
        ...
        backtracks.append(backtrack_counter)
        ...
        print('Number of unit literals:', len(units))
